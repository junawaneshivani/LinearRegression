...
if __name__ == "__main__":
    ...
    y_pred = get_predicted_y(theta0, theta1, X)                         # calculate y_pred for current values of theta
    mse = mean_squared_error(y, y_pred)                                 # calculate mean square error
    plot_graph_and_line(X, y, y_pred)
    
    for i in range(1000):
        theta0, theta1 = gradient_descent(theta0, theta1, learning_rate, X, y, y_pred)  # new values of theta
        y_pred = get_predicted_y(theta0, theta1, X)                                     # calculate y_pred for current values of theta
        new_mse = mean_squared_error(y, y_pred)                                         # calculate mean square error
